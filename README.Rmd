...
efficiently on simple workstations albeit large amounts of data. Our
workflow development has identified five major levers: indicator type, indicator intensity, focal time period, baseline time period, and spatial buffer. 
Flexibility on these five attributes should be maximized for users. Thus, the tool offers the functionality to automatically derive spatio-temporal links with other georeferenced data (e.g., surveys, digital behavioral data).
...
ground-based as well as air- and sea-borne sensors to track the Earth system and provide this information largely free for all customers.
...
There are several major topics in the social sciences which could benefit from the integration of EO: 
...
`gxc` is meant to function efficiently on simple workstations. However, if users have access to a cluster environment, you can set up a cluster-plan with the `future`-package.
...
In general, output files will have the same format as the input files. Indicator values are added as additional variables to `sf` objects and as additional layers to `SpatRaster` objects. When spatial buffer are specified for point data, the `sf`-output file contains polygons.
...
```{r setup_example1, message=FALSE, warning=FALSE}
...
```{r setup_example2, message=FALSE, warning=FALSE}
# Install and load required packages
