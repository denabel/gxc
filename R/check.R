...
#' @param temp_res Temporal resolution.
...
#' Check if a given indicator is in the allowed values for a catalogue
...
#' Check that months and time_span are not both specified
#' @param months Integer vector or NULL.
#' @param time_span Integer.
#' @returns NULL or an error.
#' @noRd
.check_months_time_span <- function(months, time_span) {
  if (!is.null(months) && time_span != 0) {
    cli::cli_abort(c(
      "{.arg months} and {.arg time_span} cannot both be specified.",
      "i" = paste(
        "Use either {.arg months} for an explicit month window",
        "or {.arg time_span} for a rolling window."
      )
    ))
  }
}
...
#' Check that months is a valid integer vector
#' @param months Integer vector or NULL.
#' @returns NULL or an error.
#' @noRd
.check_valid_months <- function(months) {
  if (!is.null(months)) {
    if (!is.numeric(months) || any(months < 1) || any(months > 12)) {
      cli::cli_abort(c(
        "Invalid {.arg months} argument.",
        "i" = "Must be an integer vector with values between 1 and 12."
      ))
    }
  }
}
