...
poly_link_daily <- function(
  ...
  with_progress({
    ...
    # Daily raster files do not have timestamp - added here
    valid_date_strings <- expand_grid(day = as.numeric(days),
    ...
    # For further operations, the time-dimension need to be stored in the raw-file
    temp_file <- file.path(path, paste0("temp_", basename(focal_path)))
    terra::writeRaster(raster, filename = temp_file, overwrite = TRUE)

    # Remove focal file and rename temp files to original name
    file.remove(focal_path)
    file.rename(temp_file, focal_path)

    temp_aux_files <- c(paste0(temp_file, ".aux.json"),
                        paste0(temp_file, ".aux.xml"))
    new_aux_files <- c(paste0(focal_path, ".aux.json"),
                       paste0(focal_path, ".aux.xml"))
    for(i in seq_along(temp_aux_files)) {
      if(file.exists(temp_aux_files[i])){
        file.rename(temp_aux_files[i], new_aux_files[i])
      }
    }
