#' Link Gridded Data with Copernicus Earth Observation Monthly Indicators
#'
#' Downloads and processes Copernicus Earth observation data (ERA5) based on gridded monthly temporal parameters,
#' and extracts the relevant monthly climate indicator values for the provided gridded dataset. The function uses the
#' time dimension of the input grid (a SpatRaster) along with specified time adjustments (time_lag and time_span, in months)
#' to construct monthly time sequences. It then downloads the corresponding monthly reanalysis data and extracts these values
#' for each grid cell. If a baseline period is specified (e.g., `baseline = c("1980", "2010")`), baseline monthly statistics are
#' downloaded for the specified period and appended as a new layer. When using the `by_hour` option, the appropriate dataset
#' for hourly-based monthly averages is used. The resampling method (e.g., `"bilinear"`) is applied to ensure that the downloaded
#' data aligns with the input grid. Parallel processing via `future.apply::future_lapply` is supported to improve performance.
#'
...
#' @param time_span Integer specifying the time span in months for averaging the climate indicator values prior to linking
#'   with the grid (default is `0`).
#' @param time_lag Integer specifying the time lag in months to shift the grid's time dimension backward (default is `0`).
...
#' @param by_hour Logical or character. If `FALSE` (default), the standard monthly averaged reanalysis is used. If a character string
#'   (e.g., `"03:00"`), the dataset for monthly averages by hour is used.
...
#' climate indicator and time period, using monthly temporal resolution. The input gridded data (a SpatRaster) is processed to determine its
#' spatial extent and its time dimension is used—after adjusting by the specified time_lag and time_span (both in months)—to build monthly
#' time sequences (with dates formatted as the first day of each month). The function then downloads the corresponding monthly data (or
#' hourly-based monthly averages if `by_hour` is specified) and extracts these values for each grid cell. If a baseline period is provided
#' (e.g., `baseline = c("1980", "2010")`), baseline monthly statistics are downloaded for the specified period, averaged over the matching
#' month, and appended as a new layer. The resampling method (e.g., `"bilinear"`) ensures that the downloaded data is aligned with the input grid.
#' Parallel processing via `future.apply::future_lapply` is supported to improve performance.
#'
...
#'
#' @return A SpatRaster containing the original grid along with appended
#' layers for the focal climate indicator values and, if requested, baseline
#' values (and optionally deviations).
#'
...
#' # Set the time dimension to a sequence of monthly dates (e.g., starting on the first day of each month)
#' terra::time(sample_grid) <- seq(as.Date("2014-01-01"), by = "1 month", length.out = nlyr(sample_grid))
#'
#' # Example: Download monthly focal values for "2m_temperature" and optionally a baseline period
  ...
  with_progress({
    ...
    grid_df$time_span_seq <- future_lapply(1:nrow(grid_df), function(i) {
      if (!is.na(grid_df[i,]$link_date) & !is.na(grid_df[i,]$link_date_end)) {
        seq_dates <- seq(grid_df[i,]$link_date_end, grid_df[i,]$link_date, by = "1 month")
    ...
    if(isFALSE(baseline)){
      ...
      if (time_span == 0) {
        ...
        # For monthly data, use the month as the identifier.
        focal_month <- month(unique_date)
        # Get baseline raster dates and extract their month.
        baseline_dates <- as.Date(terra::time(baseline_raster))
        baseline_months <- month(baseline_dates)
        # Find layers with matching months.
        match_idx <- which(baseline_months == focal_month)
        if (length(match_idx) == 0) {
          warning("No matching baseline layers found for the target month.")
          return(NA)
        }
        # Average the matched layers.
        baseline_avg <- terra::app(baseline_raster[[match_idx]],
                                   mean, na.rm = TRUE)
        # Resample to match the grid.
        baseline_extracted <- terra::resample(baseline_avg,
                                              data_sf, method = method)
        ...
        baseline_dates <- as.Date(terra::time(baseline_raster))
        baseline_months <- month(baseline_dates)
        # Find all layers whose month appears in the target sequence.
        match_idx <- which(baseline_months %in% seq_months)
        if (length(match_idx) == 0) {
